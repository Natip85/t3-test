// copied from: https://github.com/omeralpi/shadcn-phone-input
...
const CountrySelectOption = ({country, countryName, selectedCountry, onChange}: CountrySelectOptionProps) => {
  return (
    <CommandItem className='gap-2' onSelect={() => onChange(country)}>
      ...
      <span className='text-sm opacity-60'>{`+${RPNInput.getCountryCallingCode(country)}`}</span>
...
const FlagComponent = ({country, countryName}: RPNInput.FlagProps) => {
  ...
  return (
    <span className='flex h-4 w-6 overflow-hidden rounded-sm bg-foreground/20'>
      {Flag && <Flag title={countryName} {...{className: 'flex-1'}} />}
    </span>
  )
}
